...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadQuestion() {
        if (currentQuestionIndex >= questions.length) {
            ...
            window.location.href = "{% url 'progress' %}?score=" + score + "&total=" + questions.length;
...
document.addEventListener("DOMContentLoaded", function () {
    fetch("/quiz/chat/")  // Correct API endpoint
        ...
        .then(data => {
            ...
            document.getElementById("performance-message").innerText = data.performance_message;
            ...
            data.weak_topics.forEach(topic => {
                ...
                li.innerText = topic;
                weakTopics.appendChild(li);
            });

            // ✅ Now call these functions with actual data
            displayProgressChart(data.history); 
            unlockAchievements(data.score);
        })
...
async function sendMessage() {
    ...
    try {
        let response = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyCyKfP22M-YclmFo8DHuBEOjd378AfdPAo", {
            method: "POST",
            headers: {
                "Content-Type": "application/json; charset=UTF-8"
                //"Authorization": "Bearer 8e3f2af35e5b354247e92348dae585a085382ba234e155357e4d59d26cee4bb5"  // Replace with your real key
            },

            // body: JSON.stringify({
            //     contents: [
            //         { role: "system", parts: [{ text: "You are an helpful AI tutor " }] },
            ...
            //     messages: [{ role: "system", content: "You are a helpful AI tutor." }, { role: "user", content: userInput }],
            //     max_tokens: 150
            // }
        ...
        let aiResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || "Error: No response received.";

        // Update AI response in chatbox
        updateLastBotMessage(aiResponse);
    } catch (error) {
        console.error("Error fetching AI response:", error);
        updateLastBotMessage("API Error: " + error.message);
    }
...
function appendMessage(sender, message) {
    ...
    // Function to update the last bot message (useful for "Thinking..." placeholder)
function updateLastBotMessage(message) {
    let lastBotMessage = document.querySelector("#chatbox .bot-message:last-child");
    if (lastBotMessage) {
